...
let fontPx=Math.max(12,Math.min(34,Number(safeGet('reader:v17:fontPx',safeGet('reader:v16:fontPx',18)))||18));
let fontId=safeGet('reader:v17:font',safeGet('reader:v16:font','serif'));if(!FONTS[fontId])fontId='serif';
let activePane=safeGet('reader:v17:pane','text');if(!['text','voice'].includes(activePane))activePane='text';
...
function renderFonts(){
  ...
  fontGrid.querySelectorAll('[data-font]').forEach(b=>b.onclick=()=>{
    fontId=b.dataset.font;safeSet('reader:v17:font',fontId);applyFontToReader();
  });
}
...
function setSpeedIndex(idx,fromTap=false){
...
function setPane(name){
  activePane=name;safeSet('reader:v17:pane',name);
...
if(fontSlider){
  fontSlider.value=String(fontPx);
  fontSlider.oninput=()=>{fontPx=Number(fontSlider.value);safeSet('reader:v17:fontPx',fontPx);applyFontToReader()};
}
if(speedSlider){
  speedSlider.oninput=()=>setSpeedIndex(Number(speedSlider.value),false);
  speedSlider.onchange=()=>setSpeedIndex(Number(speedSlider.value),false);
}
