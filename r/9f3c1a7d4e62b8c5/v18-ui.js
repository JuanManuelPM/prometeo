...
let fontPx=Math.max(12,Math.min(34,Number(safeGet('reader:v18:fontPx',safeGet('reader:v17:fontPx',18)))||18));
let fontId=safeGet('reader:v18:font',safeGet('reader:v17:font','serif'));if(!FONTS[fontId])fontId='serif';
let activePane=safeGet('reader:v18:pane',safeGet('reader:v17:pane','text'));if(!['text','voice'].includes(activePane))activePane='text';
...
function renderFonts(){
  ...
  fontGrid.querySelectorAll('[data-font]').forEach(b=>b.onclick=()=>{fontId=b.dataset.font;safeSet('reader:v18:font',fontId);applyFontToReader()});
}
...
function setPane(name){
  activePane=name;safeSet('reader:v18:pane',name);
...
if(fontSlider){fontSlider.value=String(fontPx);fontSlider.oninput=()=>{fontPx=Number(fontSlider.value);safeSet('reader:v18:fontPx',fontPx);applyFontToReader()}}
