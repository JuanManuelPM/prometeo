...
const FONTS={
  serif:{label:'Libro',sub:'Georgia',family:'Georgia, "Times New Roman", serif'},
  sans:{label:'Limpia',sub:'Sistema',family:'Inter, system-ui, -apple-system, "Segoe UI", sans-serif'},
  classic:{label:'Clásica',sub:'Palatino',family:'"Palatino Linotype", "Book Antiqua", Palatino, serif'},
  accessible:{label:'Accesible',sub:'Verdana',family:'Verdana, Tahoma, sans-serif'}
};
const VOICE_META={julian:['Julián','Profesor'],clara:['Clara','Profesora'],vera:['Vera','Lectura'],milo:['Milo','Lectura']};
let fontPx=Math.max(12,Math.min(34,Number(safeGet('reader:v15:fontPx',18))||18));
let fontId=safeGet('reader:v15:font','serif');if(!FONTS[fontId])fontId='serif';
let activePane=safeGet('reader:v15:pane','text');if(!['text','voice'].includes(activePane))activePane='text';
...
function renderFonts(){
  if(!fontGrid)return;
  fontGrid.innerHTML=Object.entries(FONTS).map(([id,f])=>`<button type="button" class="fontChoice${id===fontId?' active':''}" data-font="${id}"><strong>${f.label}</strong><small>${f.sub}</small></button>`).join('');
  fontGrid.querySelectorAll('[data-font]').forEach(b=>b.onclick=()=>{fontId=b.dataset.font;safeSet('reader:v15:font',fontId);applyFont()});
}
...
function renderVoices(){
  ...
  voiceGrid.innerHTML=Object.entries(VOICE_META).map(([id,[name,kind]])=>`<button type="button" class="voiceChoice${id===current?' active':''}" data-voice="${id}"><strong>${name}</strong><small>${kind}</small></button>`).join('');
...
function currentSpeed(){
  const b=hiddenSpeeds?.querySelector('.choice.active');
  const n=Number(b?.dataset.s);return SPEEDS.includes(n)?n:1;
}
...
function renderSpeed(){
  ...
  if(speedTicks){speedTicks.innerHTML=SPEEDS.map((s,i)=>`<button type="button" class="speedTick${i===idx?' active':''}" data-speed-index="${i}">${String(s).replace('.',',')}×</button>`).join('');speedTicks.querySelectorAll('button').forEach(b=>b.onclick=()=>setSpeedIndex(Number(b.dataset.speedIndex),true))}
}
...
function setPane(name){
  activePane=name;safeSet('reader:v15:pane',name);
...
if(fontSlider){fontSlider.value=String(fontPx);fontSlider.oninput=()=>{fontPx=Number(fontSlider.value);safeSet('reader:v15:fontPx',fontPx);applyFont()}}
if(speedSlider){speedSlider.oninput=()=>setSpeedIndex(Number(speedSlider.value),false)}

// Keep custom controls synced with the proven engine-generated hidden controls.
