...
const root=fs.mkdtempSync(path.join(os.tmpdir(),'prometeo-eff-noalloc-'));
...
const noalloc=(id,closed,reason='BOUNDED_CLAIM_RACES_EXHAUSTED')=>write(`coordination/workers/no-allocation/${id}.json`,{
  schema:'prometeo.worker-no-allocation/v1',worker_id:id,closed_at:iso(closed),reason
});
const build=()=>{
...
for(let i=0;i<5;i++){
  ...
  const id=`old-slow-${i}`;
  beacon(id,launched);
  noalloc(id,launched+120_000);
}

const historical=build();
assert.equal(historical.metrics.no_allocation_close_p90_ms,120_000,'cumulative historical p90 must remain visible');
assert.equal(historical.metrics.no_allocation_recent,0,'old slow closures must fall outside the current regression window');
assert.equal(historical.metrics.no_allocation_close_p90_recent_ms,null);
assert.equal(historical.metrics.no_allocation_regression_window_minutes,10);
assert(!historical.reasons.includes('NO_ALLOCATION_P90_GT_90S'),'historical-only tail must not pin current regression');
...
for(let i=0;i<2;i++){
  ...
  beacon(id,launched);
  noalloc(id,launched+(100_000+i*10_000));
}
const recent=build();
assert.equal(recent.metrics.no_allocation_recent,2);
