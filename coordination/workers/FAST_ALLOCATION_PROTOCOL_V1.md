...
3b. **MEDIATED WEB ACCESS IS NOT UNRESTRICTED ORIGIN HTTP:** search/index retrieval, allowlisted web readers, repository fetchers, or a runtime whose general-purpose code execution explicitly has no network do not satisfy `unrestricted_public_http_origin_fetch`. When the current runtime exposes no tool/primitive capable of arbitrary public-origin HTTP requests with DNS resolution to the candidate target class, `unrestricted_public_http_origin_fetch` is definitively absent and MUST be classified `CAPABILITY_MISMATCH_PRECLAIM` before any authority CREATE. A genuinely arbitrary HTTP client/network tool keeps the candidate eligible; an ambiguous or undocumented network surface remains unknown rather than absent. Use only the known runtime/tool contract; do not probe/read project or target state to decide.
...
4a. **REPEATED-BOUNDARY RECOVERY EXCEPTION:** if and only if an allocator-supplied recovery candidate includes non-null `capability_confirmation_required`, that marker is evidence that the same job has already produced at least two consecutive terminal boundaries for one required capability with no intervening success or new material basis. For this marked recovery only, UNKNOWN is not enough to spend authority again: proceed only when the current runtime/tool contract gives definitive positive evidence for `capability_confirmation_required.capability`. Definitive absence OR unknown/ambiguous capability skips this candidate preclaim. Do not probe the target to manufacture positive evidence. This exception never applies to ordinary first attempts or unrelated capabilities.
...
- **IMMUTABLE FRONTIER REPLAY — RETENTION RECOVERY ONLY:** for BATCH/POOL only, if this worker already read the compact frontier through the GitHub connector, retained that exact read's immutable blob SHA, then a real `CREATE_EXISTS` requires advancing but runtime/tool orchestration no longer has the unselected contract-complete candidate objects in active context, it MAY call GitHub `fetch_blob` exactly once on that retained blob SHA solely to reconstruct the byte-identical originally observed candidate view. This is NOT a new allocator/frontier snapshot, cannot observe newer branch state, adds zero state discovery, and MUST reuse the original `beacon_commit_sha`, capability facts, candidate order and authority-attempt accounting. It is unavailable when the initial exact blob SHA was not retained; it is never allowed for initial routing, stale refresh, or after `CLAIM_TRANSPORT_BLOCKED`, `CLAIM_TRANSPORT_AMBIGUOUS`, `CLAIM_TRANSPORT_UNSTABLE`, branch-head-only movement, capability mismatch, or barrier timing. Immutable replay never raises the three-attempt base ceiling or the one permitted POOL_TAIL_RESCUE ceiling and never permits payload fabrication or directory archaeology.
...
- After `CREATE_EXISTS`, advance cyclically to the next untried candidate in that same filtered unified view, while preserving the ordinary base maximum of 3 authority CREATE attempts. If all three base attempts return `CREATE_EXISTS` and the retained compatible view still has an untried candidate, BATCH/POOL may use the single `POOL_TAIL_RESCUE` exception below. `BRANCH_HEAD_MOVED` is transport/CAS stabilization, not a candidate collision; apply the bounded same-path retry below before advancing.
...
The ordinary fast phase remains capped at 3 authority candidate CREATE attempts. For BATCH/POOL only, when exactly three real base attempts all returned `CREATE_EXISTS` and the contract-complete compatible view still contains an untried retained candidate, the worker MAY attempt exactly one fourth CREATE against the next retained untried candidate. This single `POOL_TAIL_RESCUE` uses no extra frontier read, no lane reordering, no payload reconstruction/fabrication and no directory archaeology. Total authority CREATE attempts are hard-capped at 4; equivalently, total authority CREATE attempts never exceed 4. It is forbidden for unbatched routing and after `CLAIM_TRANSPORT_BLOCKED`, `CLAIM_TRANSPORT_AMBIGUOUS`, `CLAIM_TRANSPORT_UNSTABLE`, capability mismatch or barrier timing. A fourth collision or transport failure ends the claim phase and re-enters allocation.
