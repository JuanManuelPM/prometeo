...
async function ensureEngine(jobId){
 ...
 loading=(async()=>{
  ...
  // Reliability first: base is materially lighter than small on phones.
  // Whisper is sensitive to encoder quantization, so keep encoder fp32 and decoder q4.
  const dtype={encoder_model:'fp32',decoder_model_merged:'q4'};
  const attempts=[];
  if(hasGPU) attempts.push({model:'onnx-community/whisper-base',device:'webgpu',dtype});
  attempts.push({model:'onnx-community/whisper-base',device:'wasm',dtype});
  let last=null;
  for(const a of attempts){
   try{
    post('stage',{jobId,stage:'MODEL_LOADING',detail:`${a.model.split('/').pop()} · ${a.device}`});
    ...
    selected=a;
    post('engine',{model:a.model.split('/').pop(),device:a.device,dtype:a.dtype});
    post('stage',{jobId,stage:'MODEL_READY',detail:`${a.model.split('/').pop()} · ${a.device}`});
    return transcriber;
   }catch(e){
    last=e;
...
async function run(item){
 ...
 try{
  ...
  post('stage',{jobId,stage:'INFERENCE',detail:`${selected?.model?.split('/').pop()||'Whisper'} · español`});
