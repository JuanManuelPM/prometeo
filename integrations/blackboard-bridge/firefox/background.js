...
async function bbSync(reason='manual'){
 ...
 bbRunning=(async()=>{const store=await bbStore();await bbNotify({state:'syncing',reason,at:Date.now()});const known=store.studyToken?await bbKnownCourseUrls(store.studyToken):[],queue=[...BB_ROOTS,...known],seen=new Set(),courses=new Map(),items=new Map();let pages=0,login=false;
 while(queue.length&&pages<180&&items.size<4200){const raw=queue.shift(),url=bbCanonicalPageUrl(raw);if(!url||seen.has(url))continue;seen.add(url);let p=null;try{p=await bbFetchPage(url)}catch{}if(!p)continue;if(p.password||/login|auth/i.test(new URL(p.url).pathname)){login=true;continue}pages++;for(const c of p.courses){const prev=courses.get(c.course_key);if(!prev||bbCourseTitleScore(c.title,c.course_key)>bbCourseTitleScore(prev.title,prev.course_key)||(!prev.href&&c.href))courses.set(c.course_key,{...prev,...c,href:bbGoodCourseUrl(c.href)?c.href:(prev?.href||'')})}for(const x of p.items)if(items.size<4200)items.set(x.item_key,x);for(const h0 of p.enqueue){const h=bbCanonicalPageUrl(h0);if(h&&!seen.has(h)&&!queue.includes(h)&&bbUseful(h)&&!bbFile(h,''))queue.push(h)}for(const c of p.courses){const h=bbCanonicalPageUrl(c.href);if(bbGoodCourseUrl(h)&&!seen.has(h)&&!queue.includes(h))queue.push(h)}if(pages%8===0)await bbNotify({state:'syncing',pages,courses:courses.size,items:items.size,queue:queue.length,at:Date.now()})}
