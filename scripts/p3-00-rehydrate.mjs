...
const evidence={
  schema:'prometeo.p3-00-evidence/v1',
  gate:'P3-00',
  generated_from_chat_memory:false,
  source_identities:{
    repository:'JuanManuelPM/prometeo',
    branch:git('rev-parse','--abbrev-ref','HEAD'),
    checkout_commit:git('rev-parse','HEAD'),
    bootstrap:{path:'reincarnation/BOOTSTRAP.json',git_blob_sha:hashObject('reincarnation/BOOTSTRAP.json')},
    required,
    runtime:[
      'shared/core/v1/durable.js',
      'shared/receipts/v1/ledger.js',
      'shared/reincarnation/v1/reincarnate.js'
    ].map(path=>({path,git_blob_sha:hashObject(path)}))
  },
  ledger:{ok:ledger.ok,count:ledger.count,last_hash:ledger.lastHash,last_receipt_id:docs.ledger.at(-1).id},
  state_crosscheck:{
    current_revision:docs.current_graph.revision,
    current_last_receipt:docs.current_graph.last_durable_receipt,
    dot_last_receipt:docs.dot_state.last_receipt,
    head_receipt:docs.head.receipt_id,
    candidate_receipt:docs.current_graph.pointers.candidate_current.receipt_id,
    catalog_identity:docs.current_graph.catalog_identity,
    v53_source:docs.current_graph.artifacts['navigator-v53-visible']?.source||null
  },
  wake
};
