...
const isAncestor=(a,b)=>!!a&&!!b&&git(['merge-base','--is-ancestor',a,b],true)==='';
const sha256=v=>crypto.createHash('sha256').update(v).digest('hex');
const scopePrefixes=(scopes=[])=>[...new Set(scopes.map(s=>String(s).split('*')[0].replace(/\/$/,'')).filter(Boolean))];
...
for(const ws of now.active_workstreams||[]){
  ...
  const drift={
    branch: actualBranchHead&&knownBranchHead&&actualBranchHead!==knownBranchHead?(branchDelta.length?'FAST_FORWARD_DELTA':'DIVERGED_OR_UNKNOWN'):'NONE',
    upstream_main: upstreamRelevant.length?'RELEVANT_MAIN_ACTIVITY':'NONE'
  };
  const defaultProfile=pack.execution_profile||((ws.mode==='LAB')?'DEEP':(ws.mode==='RELEASE'?'DEEP':'DEEP'));
  const packetCore={
    ...
    execution:{default_profile:defaultProfile,profiles,hard_commands:runtime.hard_commands},
  ...
  packets.push({id:ws.id,title:ws.title,mode:ws.mode,status:ws.status,url:`https://juanmanuelpm.github.io/prometeo/agent-runtime/workstreams/${ws.id}.json`,packet_hash:packetHash,actual_branch_head:actualBranchHead,drift});
...
console.log(JSON.stringify({ok:true,runtime:runtime.version,main_head:mainHead,packets:packets.map(p=>({id:p.id,drift:p.drift}))},null,2));
