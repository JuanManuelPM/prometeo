...
const data={
  ...
  scoreboard:{generated_at:scoreboard.generated_at||null,source_exam_count:scoreboard.source_exam_count||0,leader:scoreboard.leader||scoreboard.champion_candidate||null,champion:scoreboard.champion||null,champion_reproducible:!!scoreboard.champion_reproducible,reproduction_counts:scoreboard.reproduction_counts||{},active_pattern_id:scoreboard.active_pattern_id||null,active_pattern_health:scoreboard.active_pattern_health||null},
...
<h2>Experimento activo</h2><section class="row"><div><b>${esc(scoreboard.active_pattern_id||'—')}</b><span class="pill">${esc(aph?.status||'UNKNOWN')}</span></div><p>tagged=${esc(aph?.tagged_exams??0)} · ≥8=${esc(aph?.qualifying_ge_min_score??0)} · max=${esc(aph?.max_score??'—')} · mediana=${esc(aph?.median_score??'—')} · collisions avg=${esc(aph?.avg_collisions??'—')} · no-allocation=${esc(aph?.no_allocation_rate??'—')}</p></section>
