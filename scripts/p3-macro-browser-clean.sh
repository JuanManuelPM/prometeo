...
checkvp(){
  ...
  setvp "$w" "$h"
  fresh_nav
  agent-browser --session "$S" wait 120 >/dev/null
  ...
  agent-browser --session "$S" wait 430 >/dev/null
...
agent-browser --session "$S" eval 'window.__PROMETEO_V53__.left();true' >/dev/null; agent-browser --session "$S" wait 700 >/dev/null
