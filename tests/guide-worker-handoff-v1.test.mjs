...
assert.equal(policy.current_recommendation_example.recommended_approx_workers,15);
...
assert.equal(mission.current_snapshot.occupancy.recommended_additional_launches_at_snapshot,15);
...
assert.equal(head.human_contract.current_approx_workers_before_guide_return,15);
assert.equal(head.active_operating_mode.current_approx_worker_handoff,15);
assert.equal(focus.current_approx_worker_handoff,15);
assert.equal(chat.current_approx_worker_handoff,15);
