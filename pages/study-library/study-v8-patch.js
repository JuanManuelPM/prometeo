...
function openVoiceStudio8(){
 ...
 $('#previewCustom8').onclick=async()=>{const a=customArgs(),b=$('#previewCustom8');b.disabled=true;b.textContent='…';try{const key='custom:preview',p=await prepareAudio8('Bueno, probemos. Si esto suena raro, por lo menos suena raro con intención.',key,crypto.randomUUID(),null,{...a})}catch{}finally{b.disabled=false;b.textContent='▶ probar'}};
 /* preview above needs a temporary voice: use direct request instead */
...
/* patch profile buttons so the universal profile remains, voice studio lives in Walkie */
