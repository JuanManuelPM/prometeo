...
async function createVideo({channelId,storyId=null,title,hook='',targetDurationMs=30000}){if(!sb||!session)throw new Error('LOGIN_REQUIRED');const key='ui:'+crypto.randomUUID();const {data,error}=await sb.from('creator_videos').insert({owner_id:user.id,channel_id:channelId,story_id:storyId,title,hook,state:'IDEA',format:'SHORT_9_16',target_duration_ms:targetDurationMs,idempotency_key:key,max_cost_cents:0,actual_cost_cents:0,metadata:{provenance:'HUMAN_UI',free_only:true},creative:{source:'channel_ui'}}).select().single();if(error)throw error;return data}
async function produce(videoId,maxCostCents=0){return await api(`/videos/${videoId}/produce`,{method:'POST',body:{mode:'AUTO',generate_voice:false,max_cost_cents:Math.max(0,Number(maxCostCents||0))},idem:'produce:'+videoId+':v1'})}
