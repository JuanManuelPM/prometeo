(()=>{
  ...
  if(!axis||!rowsHost||!range||!prev||!next||!editor)return;
  ...
  function loadExpanded(){
    try{
      const raw=JSON.parse(localStorage.getItem(GRAPH_KEY)||'null');
      if(raw&&typeof raw==='object')return raw;
    }catch{}
    return {};
  }

  let state=load(),expanded=loadExpanded(),offset=0,selected={trackerId:null,date:null};
  ...
  function streakFor(t){
    ...
    for(let i=COUNT-1;i>=0;i--){
      ...
      const ok=t.kind==='avoid'?s==='clear':t.kind==='binary'?s==='good':s==='done';
      if(!ok)break;
      streak++;
    }
    return streak;
  ...
  function toggleMarkup(t){
    ...
    return `<button class="trace-graph-toggle" type="button" data-graph-toggle="${t.id}" aria-expanded="${open}" aria-label="${open?'Ocultar':'Mostrar'} gráfico de ${t.label}"><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M3 5.5 8 10l5-4.5"/></svg></button>`;
  }
  ...
  const W=1400,H=104,TOP=9,BOTTOM=11;
  ...
  function inlineGraphRow(t){
    ...
    const row=document.createElement('div');row.className='trace-inline-graph';row.dataset.graphFor=t.id;
    const side=document.createElement('div');side.className='trace-inline-graph-side';
    const plot=document.createElement('div');plot.className='trace-inline-graph-plot';
    ...
    let html=`<line class="trace-inline-baseline" x1="0" y1="${y(0)}" x2="${W}" y2="${y(0)}"/>`;
    if(offset===0)html+=`<line class="trace-inline-today" x1="${x(COUNT-1)}" y1="0" x2="${x(COUNT-1)}" y2="${H}"/>`;
    html+=`<path class="trace-inline-line" d="${smoothPath(values)}"/>`;
    svg.innerHTML=html;plot.append(svg);row.append(side,plot);
    return row;
  }

  function bindGraphToggles(){
    rowsHost.querySelectorAll('[data-graph-toggle]').forEach(btn=>{
      btn.onclick=e=>{
        e.preventDefault();e.stopPropagation();
        const id=btn.dataset.graphToggle;
  ...
  function renderRows(){
    ...
    for(const t of TRACKERS){
      ...
      label.innerHTML=`<span class="trace-name">${t.label}</span>${flameMarkup(streakFor(t))}${toggleMarkup(t)}`;
  ...
  function renderEditor(){
    ...
    const add=(label,value,primary=false)=>{
      ...
      b.onclick=()=>{setStatus(t.id,selected.date,value);renderRows();renderEditor();};editorActions.append(b);
    };
    if(t.kind==='avoid'){add('BIEN','clear');add('GANAS','crave');add('CAÍ','lapse',true);add('BORRAR','unknown');}
    else if(t.kind==='binary'){add('BIEN','good');add('MAL','bad',true);add('BORRAR','unknown');}
  ...
  function renderAll(){renderAxis();renderRows();renderEditor();}
  renderAll();
})();
