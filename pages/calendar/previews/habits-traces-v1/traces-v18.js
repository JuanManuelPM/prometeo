(()=>{
  /* v18 interaction invariant: day cells are the control. No editor strip.
     Repeated taps cycle through every state for that tracker and finally back to unknown. */
  const TRACKERS=[
  ...
  function streakFor(t){
    let streak=0;
    for(let i=COUNT-1;i>=0;i--){const s=status(t.id,dateAt(i)),ok=t.kind==='avoid'?s==='clear':t.kind==='binary'?s==='good':s==='done';if(!ok)break;streak++}
    return streak;
  }
  ...
  function inlineGraphRow(t){
    ...
    const side=document.createElement('div');side.className='trace-inline-graph-side';const plot=document.createElement('div');plot.className='trace-inline-graph-plot';const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
  ...
  function renderRows(){
    ...
    for(const t of TRACKERS){
      ...
      const row=document.createElement('div');row.className='trace-row';const label=document.createElement('div');label.className='trace-label';label.innerHTML=`<span class="trace-name">${t.label}</span>${flameMarkup(streakFor(t))}${toggleMarkup(t)}`;const track=document.createElement('div');track.className='trace-track';
  ...
  function renderAll(){renderAxis();renderRows();if(editor)editor.hidden=true}
  renderAll();
})();