(()=>{
  /* v23: category toggles stay intact. Graph-side stats become icon-first and the graph itself
     no longer carries the HOY separator; that temporal boundary belongs only to the register. */
  const GROUPS=[
  ...
  function bestPreviousStreak(t,current){
    ...
    if(!dates.length)return {length:0,end:null};
    ...
    while(d<=cutoff){
      if(isGood(t,status(t.id,iso(d)))){
        run++;
        if(run>=best){best=run;bestEnd=iso(d)}
      }else run=0;
      d=addDays(d,1);
    }
