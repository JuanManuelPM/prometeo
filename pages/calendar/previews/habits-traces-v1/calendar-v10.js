/* Calendar bridge v10: compatibility guard for cached calendar assets. */
(()=>{
  ...
  function normalize(){
    try{
      ...
      doc.querySelectorAll('.time-break-band').forEach(b=>{
  ...
  function install(){
    try{
      const doc=frame.contentDocument;if(!doc)return;
      if(!doc.getElementById('prometeo-calendar-v10-style')){
        const style=doc.createElement('style');style.id='prometeo-calendar-v10-style';
        style.textContent=`
          ...
          .time-break-band{
            ...
            background:var(--b)!important;color:var(--a)!important;
            border:0!important;border-radius:0!important;box-shadow:none!important;
            font-size:6px!important;font-weight:950!important;letter-spacing:.16em!important;
      ...
      mo=new MutationObserver(()=>normalize());
      if(doc.body)mo.observe(doc.body,{subtree:true,childList:true});
      if('ResizeObserver' in window){
        ro=new ResizeObserver(size);ro.observe(doc.documentElement);if(doc.body)ro.observe(doc.body);
      }
      size();setTimeout(normalize,60);setTimeout(size,220);
    }catch{}
  }
