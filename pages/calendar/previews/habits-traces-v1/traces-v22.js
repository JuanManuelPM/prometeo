(()=>{
  /* v22: categories are visible full-width toggle bands. They replace anonymous separators
     and can hide whole tracker groups without changing behavior logs or graph preferences. */
  const GROUPS=[
  ...
  const TRACKERS=[
    ...
    {id:'food',label:'Comida',group:'routine',kind:'binary'},
    {id:'train',label:'Entrenar',group:'routine',kind:'positive'},
    {id:'sleep',label:'Sueño',group:'routine',kind:'binary'},
    {id:'meds',label:'Pastillas',group:'routine',kind:'binary'},
    {id:'study',label:'Estudio',group:'focus',kind:'positive'},
    {id:'news',label:'Noticias',group:'focus',kind:'positive'}
  ];
  const CYCLES={avoid:['unknown','clear','crave','lapse'],binary:['unknown','good','bad'],positive:['unknown','done','missed']};
  ...
  let state=load(),expanded=loadObject(GRAPH_KEY),collapsed=loadObject(GROUP_KEY),offset=0;
  ...
  function cycleStatus(t,date){const cycle=CYCLES[t.kind],current=status(t.id,date),i=cycle.indexOf(current);setStatus(t.id,date,cycle[((i<0?0:i)+1)%cycle.length])}
  ...
  function currentStreak(t){let streak=0,d=new Date();while(streak<3660&&isGood(t,status(t.id,iso(d)))){streak++;d=addDays(d,-1)}return streak}
  function bestPreviousStreak(t,current){
    const dates=Object.keys(state[t.id]||{}).sort();if(!dates.length)return 0;
    const first=fromISO(dates[0]),cutoff=addDays(new Date(),-(Math.max(1,current+1)));if(first>cutoff)return 0;
    let run=0,best=0,d=new Date(first);
    while(d<=cutoff){if(isGood(t,status(t.id,iso(d)))){run++;if(run>best)best=run}else run=0;d=addDays(d,1)}
    return best;
  }
  ...
  function statsMarkup(t){const current=currentStreak(t),best=bestPreviousStreak(t,current);return `<div class="trace-inline-stats" aria-label="Racha actual ${current} días. Mejor racha anterior ${best} días."><div class="trace-stat-current">${flameSvg()}<strong>${current}</strong><span>RACHA</span></div><div class="trace-stat-best"><span>MEJOR</span><strong>${best}</strong></div></div>`}
    ...
    for(const group of GROUPS){
      ...
      for(const t of TRACKERS.filter(x=>x.group===group.id)){
        ...
        for(let i=0;i<COUNT;i++){
          ...
          button.setAttribute('aria-label',`${t.label} · ${human(date)} · ${stateLabel(s)}. Tocar para cambiar.`);
