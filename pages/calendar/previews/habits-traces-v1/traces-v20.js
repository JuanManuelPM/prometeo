(()=>{
  /* v20: day cells still cycle directly. Category labels no longer consume rows;
     group changes are zero-height dividers, and HOY is separated by one continuous boundary. */
  const TRACKERS=[
  ...
  function renderRows(){
    rowsHost.innerHTML='';
    let group=null,firstTracker=true;
    for(const t of TRACKERS){
      const startsGroup=t.group!==group;
      if(startsGroup)group=t.group;
      const row=document.createElement('div');
      row.className='trace-row'+(startsGroup?' group-start':'')+(firstTracker?' group-start-first':'');
      row.dataset.group=t.group;
