...
const CACHE_VERSION='push-scroll-lector-fusion-v3';
const MAX_CHARS=500;
const AHEAD=5;
const WORKERS=2;
...
function buildChapter(el,index){
  ...
  mount.innerHTML=`
    <div class="audio-module">
      <div class="audio-meta">
        ...
        <span class="audio-total">/ 0:00</span>
        <span class="audio-status"></span>
        <button class="audio-settings-button" type="button" aria-label="Voz y velocidad" aria-expanded="false">
          <svg viewBox="0 0 16 16" aria-hidden="true">
            <path d="M3 4h10M5.5 4a1.5 1.5 0 1 0 0 .01M3 8h10M10.5 8a1.5 1.5 0 1 0 0 .01M3 12h10M7.5 12a1.5 1.5 0 1 0 0 .01" fill="none" stroke="currentColor" stroke-width="1.35" stroke-linecap="round"/>
          </svg>
        </button>
      </div>
  ...
  return{
    ...
    el,
    mount,
    numberEl:el.querySelector('.chapter-number'),
    paragraphEls,
    paragraphs,
    offsets,
    totalChars:cursor,
    ...
    ui:{
      ...
      current:mount.querySelector('.audio-current'),
      total:mount.querySelector('.audio-total'),
...
let speed=Number(safeGet('push-reader:v3:rate',1));
if(!SPEEDS.includes(speed))speed=1;
let voice=safeGet('push-reader:v3:voice','julian');
...
function syncGeometry(ch){
  ...
  const bottom=last-base.top;
  ch.ui.track.style.top=`${top}px`;
  ch.ui.track.style.height=`${Math.max(1,bottom-top)}px`;

  const squareH=ch.numberEl.getBoundingClientRect().height;
  const metaH=ch.ui.meta.offsetHeight||54;
  const zoneStart=squareH+10;
  const zoneEnd=Math.max(zoneStart,top-10);
  let metaTop=zoneStart;

  if(zoneEnd-zoneStart>metaH){
    metaTop=zoneStart+(zoneEnd-zoneStart-metaH)/2;
  }else{
    metaTop=Math.max(squareH+6,top-metaH-8);
  }

  ch.ui.meta.style.top=`${metaTop}px`;
  ch.ui.settings.style.top=`${Math.min(top-4,metaTop+metaH+4)}px`;
}
...
function bufferedOffset(ci){
  ...
  if(clip(ci,voice,resolved.idx).status==='ready'){
    end=ch.chunks[resolved.idx].globalEnd;
  }
...
function currentSeconds(ci){
  const ch=chapters[ci];
  const ratio=clamp(ch.playhead/Math.max(1,ch.totalChars),0,1);
  const fallback=totalSeconds(ci)*ratio;
  if(ci!==state.chapter)return fallback;

  let total=0;
  for(let i=0;i<state.current;i++){
    const a=clip(ci,voice,i).audio;
    if(!a||!Number.isFinite(a.duration))return fallback;
    total+=a.duration/speed;
  }

  const currentClip=clip(ci,voice,state.current).audio;
  if(state.audio&&Number.isFinite(state.audio.currentTime)){
    total+=state.audio.currentTime/speed;
    return total;
  }
  if(currentClip&&Number.isFinite(currentClip.duration)&&state.pendingFraction!=null){
    total+=(currentClip.duration*state.pendingFraction)/speed;
    return total;
  }
  return fallback;
}
...
function updateChapterUI(ci){
  ...
  if(trackRect.height>0){
    ...
    if(by!=null){
      ui.buffer.style.height=`${clamp(by-trackRect.top,0,trackRect.height)}px`;
    }
  }

  const ratio=clamp(ch.playhead/Math.max(1,ch.totalChars),0,1);
  const tot=totalSeconds(ci);
  ui.current.textContent=fmt(currentSeconds(ci));
  ui.total.textContent=`/ ${fmt(tot)}`;
  ...
  if(loading)ui.status.textContent='preparando';
  else if(bufferedOffset(ci)>=ch.totalChars-1)ui.status.textContent='listo';
...
function maintainBuffer(){
  ...
  for(let i=state.current;i<ch.chunks.length&&slots>0;i++,slots--){
    ensure(state.chapter,voice,i,i===state.current);
  }
...
async function pump(){
  while(state.activeJobs<WORKERS&&state.queue.length){
    ...
    state.activeJobs++;
    loadClip(job.ci,job.v,job.i).finally(()=>{
      state.activeJobs--;
      pump();
    });
  }
...
async function acceptAudio(ci,v,i,c,r){
  ...
  if(ci===state.chapter&&v===voice&&i===state.current&&state.intent){
    startCurrent();
  }
}
...
function renderSettings(){
  chapters.forEach(ch=>{
    ch.ui.voices.innerHTML=Object.values(VOICES).map(v=>
      `<button type="button" class="choice${voice===v.id?' active':''}" data-voice="${v.id}">${v.name}</button>`
    ).join('');

    ch.ui.speeds.innerHTML=SPEEDS.map(v=>
      `<button type="button" class="choice${speed===v?' active':''}" data-speed="${v}">${String(v).replace('.',',')}×</button>`
    ).join('');

    ch.ui.voices.querySelectorAll('[data-voice]').forEach(b=>{
      b.onclick=e=>{
        e.stopPropagation();
    ...
    ch.ui.speeds.querySelectorAll('[data-speed]').forEach(b=>{
      b.onclick=e=>{
        e.stopPropagation();
        selectSpeed(Number(b.dataset.speed));
      };
    });
  });
}
...
function selectVoice(v){
  ...
  state.playing=false;
  voice=v;
  safeSet('push-reader:v3:voice',voice);
...
function selectSpeed(v){
  ...
  speed=v;
  safeSet('push-reader:v3:rate',speed);
  applyAllRates();
  renderSettings();
