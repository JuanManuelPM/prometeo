...
((global)=>{
  ...
  function create(config,{studentId=config.studentId||'local-default',privacy='LOCAL'}={}){
    ...
    const key=`${studentId}:${config.id}`;const record=P?.readRecord?.('student-world-state',key,{version:'1',defaultValue:null})||null;
    ...
    function persistState(state){if(!P)return null;const receipt=P.write('student-world-state',key,state,{version:'1',privacy,baseRevision:revision,meta:{worldId:config.id,studentId}});revision=receipt.revision;conflict=null;return receipt;}
    ...
    function reload(){if(!P)return engine.getState();const fresh=P.readRecord('student-world-state',key,{version:'1',defaultValue:null});if(!fresh)return engine.getState();revision=fresh.revision;engine=W.create(config,fresh.data);conflict=null;return engine.getState();}
