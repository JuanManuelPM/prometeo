...
function completedData(s:string){
  ...
  const parsed=JSON.parse(m[1]);const file=parsed?.[0];
  const url=file?.url||file?.path;if(!url)throw new Error("ZEROGPU_IMAGE_URL_MISSING");return{url:String(url),seed:parsed?.[1]??null};
}
async function generate(prompt:string,width=768,height=1024){
  width=Math.max(256,Math.min(1024,Math.round(width/32)*32));height=Math.max(256,Math.min(1536,Math.round(height/32)*32));
  const start=await fetch(`${SPACE}/gradio_api/call/infer`,{method:"POST",signal:AbortSignal.timeout(15000),headers:{"Content-Type":"application/json"},body:JSON.stringify({data:[prompt,0,true,width,height,4]})});
  const first:any=await start.json().catch(()=>({}));if(!start.ok||!first.event_id)throw new Error(`ZEROGPU_SUBMIT_${start.status}`);
  const poll=await fetch(`${SPACE}/gradio_api/call/infer/${encodeURIComponent(first.event_id)}`,{signal:AbortSignal.timeout(90000),headers:{Accept:"text/event-stream"}});
  ...
  const image=await fetch(fileUrl,{signal:AbortSignal.timeout(30000)});if(!image.ok)throw new Error(`ZEROGPU_FETCH_${image.status}`);const bytes=new Uint8Array(await image.arrayBuffer());if(bytes.byteLength<1000)throw new Error("ZEROGPU_IMAGE_TOO_SMALL");
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const out=await generate(prompt,Number(b.width||768),Number(b.height||1024));
