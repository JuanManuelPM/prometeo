...
async function probe(provider:string,credentials:any){
  if(provider==="huggingface"||provider==="huggingface_zerogpu"){
    const token=String(credentials.token||"");if(!token)return{supported:true,ok:false,error:"TOKEN_REQUIRED",free_safe:false,router_enabled:false};
    const r=await fetchJson("https://huggingface.co/api/whoami-v2",{headers:{Authorization:`Bearer ${token}`}});
    const zero=provider==="huggingface_zerogpu";
    return{supported:true,ok:r.ok,status:r.status,account:r.data?.name||null,error:r.ok?null:(r.data?.error||"HF_PROBE_FAILED"),free_safe:zero&&r.ok,router_enabled:false,quota_booster:zero&&r.ok,guard:zero&&r.ok?"ZEROGPU_TOKEN_VERIFIED":"HF_ACCOUNT_VERIFIED"};
  }
  if(provider==="cloudflare"){
    const token=String(credentials.api_token||""),account=String(credentials.account_id||"");if(!token||!account)return{supported:true,ok:false,error:"ACCOUNT_ID_AND_TOKEN_REQUIRED",router_enabled:false};
    const h={Authorization:`Bearer ${token}`};const t=await fetchJson("https://api.cloudflare.com/client/v4/user/tokens/verify",{headers:h});if(!t.ok)return{supported:true,ok:false,status:t.status,error:"CLOUDFLARE_TOKEN_INVALID",free_safe:false,router_enabled:false};
    const a=await fetchJson(`https://api.cloudflare.com/client/v4/accounts/${encodeURIComponent(account)}`,{headers:h});if(!a.ok)return{supported:true,ok:false,status:a.status,error:"CLOUDFLARE_ACCOUNT_UNREADABLE",free_safe:false,router_enabled:false};
    ...
    return{supported:true,ok:true,status:a.status,account_id:account,token_ok:true,account_ok:true,billing_probe_ok:s.ok,free_safe:freeSafe,router_enabled:freeSafe,guard,error:null};
  }
  if(provider==="pollinations"){const key=String(credentials.api_key||"");if(!key)return{supported:true,ok:false,error:"API_KEY_REQUIRED",router_enabled:false};const r=await fetchJson("https://gen.pollinations.ai/v1/models",{headers:{Authorization:`Bearer ${key}`}});return{supported:true,ok:r.ok,status:r.status,error:r.ok?null:"POLLINATIONS_KEY_INVALID",free_safe:false,router_enabled:false}}
  if(provider==="together"){const key=String(credentials.api_key||"");if(!key)return{supported:true,ok:false,error:"API_KEY_REQUIRED",router_enabled:false};const r=await fetchJson("https://api.together.xyz/v1/models",{headers:{Authorization:`Bearer ${key}`}});return{supported:true,ok:r.ok,status:r.status,error:r.ok?null:"TOGETHER_KEY_INVALID",free_safe:false,router_enabled:false}}
  if(provider==="replicate"){const key=String(credentials.api_token||"");if(!key)return{supported:true,ok:false,error:"API_TOKEN_REQUIRED",router_enabled:false};const r=await fetchJson("https://api.replicate.com/v1/account",{headers:{Authorization:`Bearer ${key}`}});return{supported:true,ok:r.ok,status:r.status,account:r.data?.username||null,error:r.ok?null:"REPLICATE_KEY_INVALID",free_safe:false,router_enabled:false}}
  return{supported:false,ok:false,error:"PROBE_NOT_IMPLEMENTED",free_safe:false,router_enabled:false};
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(m&&req.method==="POST"){
      ...
      const c=await upsertConnection(ownerId,provider,{mode,vault_secret_id:sid,external_account_id:String((creds as any).account_id||pr.account||"")||null,verified_at:pr.ok?now():null,last_probe:pr,last_error:pr.supported&&!pr.ok?pr:null,metadata:{capabilities:cat.capabilities,free_policy:cat.free_policy,free_safe:pr.free_safe===true,router_enabled:pr.router_enabled===true,quota_booster:pr.quota_booster===true,configured_inside:"channels",secret_echoed:false,guard:pr.guard||null}});
      return json({ok:mode!=="BLOCKED",connection:publicConn(c),probe:pr},mode==="BLOCKED"?422:200);
    }
    m=path.match(/^\/providers\/([a-z0-9_\-]+)\/probe$/i);
    if(m&&req.method==="POST"){
      const provider=m[1],c=await connection(ownerId,provider);if(!c?.vault_secret_id)return bad("PROVIDER_NOT_CONNECTED",409);const raw=await vaultRead(c.vault_secret_id);let creds:any={};try{creds=JSON.parse(raw)}catch{creds={api_key:raw,token:raw,api_token:raw}}const pr=await probe(provider,creds);const next=await upsertConnection(ownerId,provider,{mode:pr.supported?(pr.ok?"VERIFIED_REAL":"BLOCKED"):c.mode,verified_at:pr.ok?now():c.verified_at,last_probe:pr,last_error:pr.supported&&!pr.ok?pr:null,metadata:{free_safe:pr.free_safe===true,router_enabled:pr.router_enabled===true,quota_booster:pr.quota_booster===true,guard:pr.guard||c.metadata?.guard||null}});return json({ok:pr.ok||!pr.supported,probe:pr,connection:publicConn(next)},pr.supported&&!pr.ok?422:200);
    }
