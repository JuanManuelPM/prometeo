...
async function generate(prompt:string,duration=2,width=288,height=512){
  ...
  const payload={data:[prompt,negative,null,null,height,width,'text-to-video',duration,9,42,true,1.0,true]};
  const start=await fetch(`${SPACE}/gradio_api/call/text_to_video`,{method:'POST',signal:AbortSignal.timeout(15000),headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
  const first:any=await start.json().catch(()=>({}));if(!start.ok||!first.event_id)throw new Error(`ZEROGPU_SUBMIT_${start.status}`);
  const poll=await fetch(`${SPACE}/gradio_api/call/text_to_video/${encodeURIComponent(first.event_id)}`,{signal:AbortSignal.timeout(150000),headers:{Accept:'text/event-stream'}});
  ...
  return{bytes,mime:vr.headers.get('content-type')||'video/mp4',seed:done.seed,event_id:first.event_id,duration,width,height};
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const out=await generate(prompt,Number(b.duration||2),Number(b.width||288),Number(b.height||512));
    ...
    const {data:vv,error:vve}=await db.from('creator_video_versions').insert({owner_id:ownerId,video_id:videoId,version,master_asset_id:asset.id,scene_manifest:[{kind:'generated_master',provider:'huggingface-ltx-zerogpu-anon',duration_ms:Math.round(out.duration*1000),width:out.width,height:out.height}],selected:true}).select().single();if(vve)throw vve;
    const findings=['mp4 atoms ftyp/moov/mdat present',`${out.width}x${out.height}`,`duration target ${out.duration}s`];
    await db.from('creator_validation_runs').insert({owner_id:ownerId,video_id:videoId,video_version_id:vv.id,kind:'MEDIA_QA',status:'PASS',score:1,findings,evidence:{provider:'huggingface-ltx-zerogpu-anon',byte_size:out.bytes.byteLength,width:out.width,height:out.height,duration_ms:Math.round(out.duration*1000),cost_cents:0}});
    const metadata={...(v.metadata||{}),last_master_asset_id:asset.id,last_master_provider:'huggingface-ltx-zerogpu-anon',last_video_provider:'huggingface-ltx-zerogpu-anon',free_only:true};
    ...
    return json({ok:true,provider:'huggingface-ltx-zerogpu-anon',cost_cents:0,video_id:videoId,video_version_id:vv.id,asset:{id:asset.id,mime_type:'video/mp4',byte_size:out.bytes.byteLength,duration_ms:Math.round(out.duration*1000)},signed_url:signed.signedUrl,qa:{status:'PASS',findings},quota:'anonymous ZeroGPU'});
