...
Deno.serve(async(req:Request)=>{try{
  ...
  if(action==="list"){
    ...
    return json(req,{ok:true,timezone:"America/Argentina/Buenos_Aires",now_minute,blocks,active_blocks,revision,runtime_build:"32"});
  }
  if(action==="state"){
    ...
    if(q.error)throw q.error;
    const states=q.data||[];
    return json(req,{ok:true,state:states[0]||null,states,runtime_build:"32"});
  }
  if(action==="tv_state"){
  ...
  if(action==="command"){
    ...
    if(!["play","pause","toggle","mute","unmute","refresh_panel","reload_when_idle","reload_now","widget_previous","widget_next","widget_toggle","widget_reset","seek_relative","seek_absolute"].includes(command))fail("COMMAND_INVALID");
