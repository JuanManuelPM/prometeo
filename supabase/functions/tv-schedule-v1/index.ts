...
Deno.serve(async(req:Request)=>{try{
  ...
  if(action==="list"){
    ...
    return json(req,{ok:true,timezone:"America/Argentina/Buenos_Aires",now_minute,blocks,active_blocks,revision,runtime_build:"36"});
  }
  if(action==="state"){
    ...
    if(q.error)throw q.error;
    const states=q.data||[];
    return json(req,{ok:true,state:states[0]||null,states,runtime_build:"36"});
  }
  if(action==="tv_state"){
