-- AUTO-Q01149 · state-truth: active Prometeo jobs require an unexpired lease.
-- Reversible projection-only change: replace this view with the prior definition.
...
select
  ...
  count(j.*)::integer as total_units,
  count(j.*) filter(where j.status='DONE')::integer as done_units,
  count(j.*) filter(where j.status='LEASED' and j.lease_expires_at>now())::integer as active_units,
  count(j.*) filter(where j.status='READY')::integer as ready_units,
  count(j.*) filter(where j.status='BLOCKED')::integer as blocked_units,
  p.created_at,p.started_at,p.finished_at
...
select
  ...
  (select count(*)::integer from public.prometeo_jobs where status='LEASED' and lease_expires_at>now()) as active_units,
