-- AUTO-Q01143 · remove duplicate BACKLOG work from Work Reservoir
-- Productive Frontier is the canonical durable-backlog materializer.

create or replace function public.prometeo_reservoir_enabled_classes()
...
as $classes$
  values
    ...
    ('REGRESSION'::text),
    ('OUTPUT_REVIEW'::text),
    ('RUNTIME_KNOWLEDGE'::text),
    ('DEPENDENCY'::text),
    ('SECURITY_TOOLING'::text),
    ('DOCS_RECONCILE'::text),
    ('RESERVOIR_QUALITY'::text);
$classes$;
...
begin
  ...
  for v_class in
    select x.class_name
    from public.prometeo_reservoir_enabled_classes() x
    where not exists(
      select 1
    ...
    and (
      x.class_name <> 'RESERVOIR_QUALITY'
      or not exists(
        select 1
        from public.prometeo_jobs q
        where q.project_id='WORK-RESERVOIR-01'
          and q.status='DONE'
          and coalesce((q.input_context->>'autofill')::boolean,false)=true
          and q.input_context->>'reservoir_class'='RESERVOIR_QUALITY'
          and q.completed_at > clock_timestamp() - interval '1800 seconds'
      )
    )
  ...
  return jsonb_build_object(
    ...
    'dedupe_mode','ONE_ACTIVE_AUTOFILL_PER_CLASS',
    'quality_cooldown_seconds',1800,
    'server_time',clock_timestamp()
  );
end;
$function$;

create or replace function public.prometeo_reservoir_classes_smoke_test()
returns jsonb
language plpgsql
stable
security definer
...
begin
  select count(*),bool_or(class_name='BACKLOG')
  into v_count,v_has_backlog
  from public.prometeo_reservoir_enabled_classes();

  if v_count<>9 or coalesce(v_has_backlog,false) then
    raise exception 'reservoir class set assertion failed: count %, backlog %',v_count,v_has_backlog;
  end if;

  return jsonb_build_object(
    'ok',true,
    'state','RESERVOIR_CLASSES_SMOKE_OK',
    'enabled_count',v_count,
    'backlog_enabled',false,
    'canonical_backlog_owner','PRODUCTIVE-FRONTIER-01'
  );
end;
$smoke$;

select public.prometeo_reservoir_classes_smoke_test();
